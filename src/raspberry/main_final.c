...
void read_sensors(){
	...
	char fim[16] = "}";
	char inicio[150] = "g02pb3EGK {";
	char hum_string[25];
	char temp_string[25];
	char lum_string[25];
	char press_string[25];

	char time_string[40];
	char time_stamp[50];	
	
	char interval_string[50];

	strftime(time_string, 48, "%Y-%m-%d %H:%M:%S", ptm);
	...
	int reconnect_pub = mosquitto_reconnect(mosq_pub);
	mosquitto_publish(mosq_pub, NULL, "G02_THEBESTGROUP/MEDICOES", 150, inicio, 0, false);
...
// função chamada quando ocorre conexão com o client
...
void on_connect(struct mosquitto *mosq, void *obj, int connect) {
	if(connect!=0){ 
		...
		lcdPrintf(lcd, "ERRO:            ");
		lcdPosition(lcd, 0, 1);
		lcdPrintf(lcd, "CONEXÃO            ", delay_time);
		exit(-1);
	}
...
void on_message(struct mosquito *mosq, void *obj, const struct mosquitto_message *msg){
	...
	char code[9];
	strncpy(code, msg->payload, 9);
	
	if(strcmp(code, "g02pb3EGK") == 0){
		char interval[16];
		strncpy(interval, msg->payload+22, 36);
		int i;
		...
		interval[i] = "\0";
		int new_delay = atoi(interval) * 1000;
		delay_time = new_delay;
		printf("\nIntervalo de medida mudado para (%d)\n", new_delay); 	
	}
}
...
int main(){
	...
	connect_pub = mosquitto_connect(mosq_pub, "10.0.0.101", 1883, 60);	

	if(user_pub != 0 || user_sub != 0 || connect_sub != 0){
		lcdPosition(lcd, 0, 0);
		lcdPrintf(lcd, "ERRO:              ");
		lcdPosition(lcd, 0, 1);
		lcdPrintf(lcd, "BROKER             ");
	...
	pthread_create(&s_thread, NULL, subscriber_thread, NULL);
	int loop = 1;
    read_sensors();
